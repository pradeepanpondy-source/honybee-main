    ...
    try {
        ...
        try {
            ...
            if (jwtError.name === 'TokenExpiredError') {
                return res.status(400).json({ error: 'Verification link has expired. Please request a new one.', valid: false, expired: true });
            }
        ...
        if (fetchError || !profile) {
            return res.status(400).json({ error: 'Verification link is invalid or has already been used.', valid: false });
        }
